...
from evaluation.box_util import box3d_iou, calc_iou, get_3d_box
    ...
    for d in range(nd):
        ...
        if BBGT.size > 0:
            ...
            for j in range(BBGT.shape[0]):
                box_pair = (bb, BBGT[j,...])
                if get_iou_func == get_iou_obb:
                    box_pair = (get_3d_box(bb[3:6], bb[7], bb[0:3]), get_3d_box(BBGT[j,...][3:6], BBGT[j,...][7], BBGT[j,...][0:3]))
                iou = get_iou_main(get_iou_func, box_pair)
                if iou > ovmax:
                    ovmax = iou
                    jmax = j
