...
""" Helper functions for calculating 2D and 3D bounding box IoU.
    Collected and written by Jason Ren & Charles R. Qi
"""
...
def batch_box3d_vol_pytorch(corners):
    ''' corners: (8,3) no assumption on axis direction '''
    a = torch.sqrt(torch.sum((corners[:, :, 0, :] - corners[:, :, 1, :])**2, dim=-1))
    b = torch.sqrt(torch.sum((corners[:, :, 1, :] - corners[:, :, 2, :])**2, dim=-1))
    c = torch.sqrt(torch.sum((corners[:, :, 0, :] - corners[:, :, 4, :])**2, dim=-1))
    return a*b*c
...
def get_3d_box_pytorch(box_size, center):
    ''' box_size is array(l,w,h), heading_angle is radius clockwise from pos x axis, center is xyz of box center
        output (8,3) array for 3D box corners
        Similar to utils/compute_orientation_3d
        TODO: axis-aligned only (B,num_proposal); no rotation
    '''
    l, w, h = box_size
    x_corners = [l/2,  l/2, -l/2, -l/2,  l/2,  l/2, -l/2, -l/2]
    y_corners = [h/2,  h/2,  h/2,  h/2, -h/2, -h/2, -h/2, -h/2]
    z_corners = [w/2, -w/2, -w/2,  w/2,  w/2, -w/2, -w/2,  w/2]
    corners_3d = torch.tensor([x_corners, y_corners, z_corners], device=center.device)
    corners_3d[0, :] = corners_3d[0, :] + center[0]
    corners_3d[1, :] = corners_3d[1, :] + center[1]
    corners_3d[2, :] = corners_3d[2, :] + center[2]
    return corners_3d.transpose(0, 1)
