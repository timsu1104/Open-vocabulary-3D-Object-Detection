...
class PRCalculator(object):
    ...
    def __init__(self, ap_iou_thresh=0.25, class2type_map=None, obb=False):
        """
            Args:
    ...
    def compute_metrics(self):
        ...
        for key in sorted(ap.keys()):
            ...
            try:
                ret_dict['%s Precision'%(clsname)] = prec[key][-1]
                prec_list.append(prec[key][-1])
            except:
                ret_dict['%s Precision'%(clsname)] = 0
                # prec_list.append(0)
        # prec_list = [prec for prec in prec_list if prec > 0]
        ret_dict['mPrecision'] = np.mean(prec_list)
        rec_list = []
        for key in sorted(ap.keys()):
            clsname = self.class2type_map[key] if self.class2type_map else str(key)
            try:
